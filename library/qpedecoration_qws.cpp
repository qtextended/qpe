...
** not clear to you.
**
** $Id: qpedecoration_qws.cpp,v 1.25 2001/08/29 13:14:00 warwick Exp $
**
**********************************************************************/
...
void QPEManager::drawButton( QWidget *w, QPEDecoration::QPERegion r, int state )
{
    ...
    painter.internalGfx()->setWidgetRegion( rgn );
    ...
    : QWSDefaultDecoration()
{
    ...
    helpFile =QPEApplication::qpeDir() + "/help/html/" + QString(qApp->argv()[0]) + ".html";
    helpExists = QFile::exists( helpFile );
...
//    int bw = rect.isEmpty() ? 0 : BORDER_WIDTH;
	...
	case Close: {
		...
		int btnWidth = imageOk.width();
		if ( widget->inherits( "QDialog" ) )
		    btnWidth += imageClose.width();
	...
	case Help: {
	    if ( helpExists ) {
		QRect r;
		int l = rect.right() - imageOk.width() - imageHelp.width();
		if ( widget->inherits( "QDialog" ) )
		    l -= imageClose.width();
...
#define CLAMP(x, y)	    ( ((x) > (y)) ? (y) : (x) )
	    ...
	    painter->drawLine( rect.left() - BORDER_WIDTH + 2,
				...
				rect.bottom() + 1 );
#if QT_VERSION >= 300
	    style.drawPrimitive( QStyle::PE_Separator, painter,
			QRect(rect.right() - CORNER_GRAB,
			    rect.bottom() + 1, rect.right() - CORNER_GRAB,
			    rect.bottom() + BOTTOM_BORDER_WIDTH),
			    cg, QStyle::Style_Sunken );
#else
	    style.drawSeparator( painter, rect.right() - CORNER_GRAB,
			    rect.bottom() + 1, rect.right() - CORNER_GRAB,
			    rect.bottom() + BOTTOM_BORDER_WIDTH,
			    cg, TRUE, 1, 0 );
#endif
