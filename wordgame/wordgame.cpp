...
** not clear to you.
**
** $Id: wordgame.cpp,v 1.16 2001/08/29 08:32:16 warwick Exp $
**
**********************************************************************/
...
static const char* mime_wgr = "text/x-wordgamerules";
...
void WordGame::writeConfig()
{
    ...
    if ( !gameover ) {
	cfg.writeEntry("Rules",rules.linkFile());
	bag->writeConfig(cfg);
...
void WordGame::readConfig()
{
    ...
    if ( currentplayer ) {
	gameover = FALSE;
	rules = DocLnk(cfg.readEntry("Rules"));
	if ( loadRules(rules) ) {
	    startGame(pnames);
	    bag->readConfig(cfg);
...
{
    rules = *newgame->ruleslist.at(newgame->rules->currentItem());
    if ( loadRules(rules) ) {
	QStringList names;
...
bool WordGame::loadRules(const DocLnk& doc)
{
    setCaption( doc.name() );
    QByteArray ba; // ### encapsulate this bit in FileManager
    FileManager fm;
    if ( !fm.loadFile(doc,ba) )
	return FALSE;
...
void NewGame::updateRuleSets()
{
    ...
    DocLnkSet folder( dir, mime_wgr );
    ruleslist = folder.children();
    QListIterator<DocLnk> it( ruleslist );
    if ( !it ) {
	// Provide a sample
	DocLnk f;
	f.setType(mime_wgr);
	f.setName("Sample");
	QByteArray ba(strlen(sampleWGR));
	memcpy(ba.data(),sampleWGR,strlen(sampleWGR));
	fm.saveFile(f,ba);
	updateRuleSets();
	return;
    }
    ...
    for ( ; it.current(); ++it ) {
	QFileInfo fi((*it)->file());
	int age = fi.lastModified().secsTo(now);
	rules->insertItem((*it)->name());
	if ( age < newest_age ) {
	    newest_age = age;
	    newest = rules->count()-1;
	}
	folder.remove(*it); // we own it now.
    }
    ruleslist.setAutoDelete(TRUE);
