...
//    setPalette(QPalette(QColor(200,200,200))); // Gray
    ...
    picks->initialise();
    QObject::connect( picks, SIGNAL(key(ushort,ushort,ushort) ),
            this, SIGNAL(key(ushort,ushort,ushort)) );
}

void Keyboard::resizeEvent(QResizeEvent*)
{
    int ph = picks->fontMetrics().lineSpacing()+3;
...
void  KeyboardConfig::generateText(const QString &s)
{
	...
	parent->emitKey( 0, Qt::Key_Backspace, 0 );
    for (int i=0; i<(int)s.length(); i++)
	parent->emitKey( s[i].unicode(), 0, 0 );
    parent->emitKey( 0, Qt::Key_Space, 0 );
    backspaces = 0;
#endif
}
...
*/
		...
		if (!blank) {
			...
			p.drawPixmap( x + 1, y + 2, QPixmap((const char**)xpm) );
    			...
    			p.setPen(textcolor);
			p.drawText( x - 1, y+1, kw, keyHeight-2, AlignCenter, s );
		    }
...
*/
	...
	if (dc) {
	    if (qk == Qt::Key_Backspace) {
		dc->input.remove(dc->input.last()); // remove last input
		dc->decBackspaces();
	    } else if ( k == 0226 /* Reset Dictionary */ ) {
		dc->input.clear();
		dc->resetBackspaces();
	    } else if (qk == Qt::Key_Return) {
		dc->input.clear();
		dc->resetBackspaces();
	    } else if (qk == Qt::Key_Space) {
		dc->input.clear();
		dc->resetBackspaces();
	    } else {
		dc->add(QString(QChar(u)));
		dc->incBackspaces();
	    }
	}
...
QSize Keyboard::sizeHint() const
{
    ...
    int keyHeight = fm.lineSpacing()+4;

    if (useOptiKeys)
    	keyHeight += 1;
    
    return QSize( 320, keyHeight * 5 + picks->height() + 1 );
}
