                     ...
                     :  QCanvasView(&c, parent, name, wFlags)
{
    ...
    if (c.readNumEntry("Answer0") < 0) {
        ...
        c.setGroup("Board");
        for(i = 0; i < 4; i++) 
            answer[i] = c.readNumEntry(tr("Answer%1").arg(i));
        /* read, and parse past guesses */
        current_go = 0;
        for(j=0; j < 9; j++) {
            current_guess[0] = c.readNumEntry(tr("Go%1p0").arg(j));
            if (current_guess[0] < 0)
                break;
            placeGuessPeg(0, current_guess[0]);
            current_guess[1] = c.readNumEntry(tr("Go%1p1").arg(j));
            placeGuessPeg(1, current_guess[1]);
            current_guess[2] = c.readNumEntry(tr("Go%1p2").arg(j));
            placeGuessPeg(2, current_guess[2]);
            current_guess[3] = c.readNumEntry(tr("Go%1p3").arg(j));
            placeGuessPeg(3, current_guess[3]);
            checkGuess();
        }
        for(i = 0; i < 4; i++) {
            current_guess[i] = c.readNumEntry(tr("CurrentGo%1").arg(i));
...
void MindBreakerBoard::resetScore() 
{
    ...
    if (QMessageBox::information(this, "Reset Statistics", 
            "Reset the win ratio?",
            "OK", "Cancel") == 0) {
        total_turns = 0;
        total_games = 0;
