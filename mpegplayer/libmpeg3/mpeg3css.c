...
static void mpeg3_generate_bits(unsigned char *output, int len, struct mpeg3_block const *s)
{
 ...
 * generators.
 */
static void mpeg3_css_engine(int varient, unsigned char const *input, struct mpeg3_block *output)
{
 ...
 */
	...
	mpeg3_generate_bits(&bits[29], sizeof bits, &temp1);
...
void mpeg3_crypt_key1(mpeg3_css_t *css, int varient, unsigned char const *challenge, struct mpeg3_block *key)
{
	...
	mpeg3_css_engine(varient, scratch, key);
}
 ...
 *                0 -> !1
 */
void mpeg3_crypt_key2(mpeg3_css_t *css, int varient, unsigned char const *challenge, struct mpeg3_block *key)
{
	...
	mpeg3_css_engine(perm_varient[varient], scratch, key);
}
 ...
 *                0 ->  3
 */
void mpeg3_crypt_bus_key(mpeg3_css_t *css, int varient, unsigned char const *challenge, struct mpeg3_block *key)
{
...
int mpeg3_get_asf(mpeg3_css_t *css)
{
	dvd_authinfo ai;
...
int mpeg3_authenticate_drive(mpeg3_css_t *css, const unsigned char *key)
{
	int i;
	...
	for(i = 0; i < 32; ++i)
	{
		mpeg3_crypt_key1(css, i, css->challenge, &(css->keycheck));
...
int mpeg3_hostauth(mpeg3_css_t *css, dvd_authinfo *ai)
{
	int i;
...
/* 			ai->lsk.key[4], ai->lsk.key[3], ai->lsk.key[2], ai->lsk.key[1], ai->lsk.key[0]); */
			if(mpeg3_authenticate_drive(css, ai->lsk.key)) 
			{
		...
		case DVD_LU_SEND_CHALLENGE:
			...
			mpeg3_crypt_key2(css, css->varient, css->challenge, &(css->key2));
...
int mpeg3_get_title_key(mpeg3_css_t *css, int agid, int lba, unsigned char *key)
{
	dvd_authinfo ai;
	int i;
...
int mpeg3_get_disk_key(mpeg3_css_t *css, int agid, unsigned char *key)
{
	dvd_struct s;
	int	index, i;
...
int mpeg3_validate(mpeg3_css_t *css, int lba, int do_title)
{
	dvd_authinfo ai;
	dvd_struct dvds;
	int result = 0;
...
/* Init sequence, request AGID */
	for(tries = 1, rv = -1; rv == -1 && tries < 4; tries++)
	{
		...
		if(rv == -1)
		{
/*			perror("mpeg3_validate: request AGID"); */
...
/* Send AGID to host */
	if(mpeg3_hostauth(css, &ai)) return 1;
...
/* Send key2 to LU */
	...
	{
		perror("mpeg3_validate: Send key2 to LU");
		return 1;
	}

	if(ai.type == DVD_AUTH_FAILURE)
	{
		fprintf(stderr, "mpeg3_validate: authorization failed\n");
		return 1;
	}
	memcpy(css->challenge, css->key1.b, 5);
	memcpy(css->challenge + 5, css->key2.b, 5);
	mpeg3_crypt_bus_key(css, css->varient, css->challenge, &(css->keycheck));
...
/* Definitely encrypted if we got here. */
	...
	if(do_title)
		return mpeg3_get_title_key(css, agid, lba, css->keycheck.b);
	else
		return mpeg3_get_disk_key(css, agid, css->keycheck.b);

	return 0;
}

int mpeg3_validate_path(mpeg3_css_t *css, int do_title)
{
	int result = 0;
	int lba = 0, file_fd;
	...
	if(do_title)
	{
		...
		{
			perror("mpeg3_validate_path: open");
			return 1;
		}

		if(ioctl(file_fd, FIBMAP, &lba) != 0)
		{
			perror("mpeg3_validate_path: FIBMAP");
			close(file_fd);
			return 1;
		}
	...
	if(!result) result = mpeg3_validate(css, lba, do_title);
 ...
 *
 */
static void mpeg3_title_key(unsigned char *key, unsigned char *im, unsigned char invert)
{
...
int mpeg3_decrypt_title_key(mpeg3_css_t *css, unsigned char *dkey, unsigned char *tkey)
{
	...
	for(i = 0; mpeg3_playkeys[i]; i++)
	{
		...
		mpeg3_title_key(pretkey, mpeg3_playkeys[i]->key, 0);

		memcpy(test, dkey, 5);
		mpeg3_title_key(test, pretkey, 0);
	...
	mpeg3_title_key(css->title_key, pretkey, 0xff);
...
void mpeg3_descramble(unsigned char *sec, unsigned char *key)
{
	unsigned int lfsr1_lo, lfsr1_hi, lfsr0, combined;
	unsigned char o_lfsr0, o_lfsr1;
...
{
	mpeg3_css_t *css = (mpeg3_css_t*)calloc(1, sizeof(mpeg3_css_t));
	css->varient = -1;
	return css;
}
...
/* Get disk key */
	result = mpeg3_validate_path(css, 0);
/* Get title key */
	if(!result) result = mpeg3_validate_path(css, 1);
/* Descramble the title key */
	if(!result) result = mpeg3_decrypt_title_key(css, css->disk_key, css->title_key);
...
{
	if(!css->encrypted) return 1;     /* Couldn't decrypt */
	mpeg3_descramble(sector, css->title_key);
	return 0;
}
